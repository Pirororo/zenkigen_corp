...
export default class Ring extends THREE.Object3D {
  ...
  constructor() {
    ...
    let Params = function(){
      ...
      this.radius = 185;
      this.span = 14;
      this.noise_step = 0.12;
      this.noise_min = 0.65;
      this.noise_max = 0.80;
      this.posX_cos_rad = 4.0;//
      this.posY_cos_rad = 4.6;
      this.posZ_sin_rad = 3.1;
      this.posZ_cos_rad = 1.2;
      // speed
      this.sin_freq = 0.01;
      this.cos_freq = 0.1
      this.noise_speed = 0.015;
      this.rotateY_speed = 0.0022;
      this.rotateZ_speed = 0.001;
    }
    ...
    var folder1 = gui.addFolder('size');
    ...
    var folder2 = gui.addFolder('speed');
        ...
        folder2.add( this.params, 'cos_freq', 0, 1 ).onChange( this.datUpdate );
  ...
  datUpdate() {
    ...
    this.cos_freq = this.params.cos_freq;
    ...
    if(this.frame %1 ==0){
      ...
      for (let i = 0; i < this.noise_seed_list.length; i++) {
        for (let deg = 0; deg < 360; deg += 1) {
          ...
          this.posX_cos_rad = this.sin_freq +2;
          this.positions[posNum] = this.radius *noise_param * Math.sin(this.posX_cos_rad * deg * Math.PI/180);
          // this.colors[posNum] = 0.5;
          posNum +=1;
          // this.posY_cos_rad = 2 *Math.cos(this.cos_freq* this.frame* Math.PI/180)+2;
          this.positions[posNum] = this.radius *noise_param * Math.cos(this.posY_cos_rad * deg * Math.PI/180);
          // this.colors[posNum] = 0.8;
          posNum +=1;
      ...
      for (let i = 3; i < this.positions.length-3; i+=3) {
        for (let k = i +3; k < this.positions.length-0; k+=3) {
